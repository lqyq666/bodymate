...
4. **MoonBit boundary (3:10–4:10).** Follow input → MoonBit action/validation/state → versioned snapshot or pose intent → JavaScript adapter → Three.js. MoonBit decides what; Three.js decides how it is displayed.
...
7. **Tests and offline delivery (6:20–7:25).** Run `npm run check`; explain MoonBit tests, cross-language contracts, rig/parameter geometry tests, frozen GLB hashes, generated-artifact checks, and clean-clone verification.
