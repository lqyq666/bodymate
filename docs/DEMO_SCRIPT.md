...
“BodyMate 是四个可以单独 `moon add` 的 MoonBit 库，加一个用它们做出来的全身动作实验室。`agent` 把大模型的提议约束成白名单里的命令——模型说手距 2.5 倍肩宽，护栏截到 1.8 并告诉你为什么；`zhnum` 让‘一点五倍’‘六十度’‘百分之八十’这样的中文数字直接可用；`anatomy` 给 697 个人体结构提供中英文名称与检索；`motion` 管动作参数、独立播放会话和确定性姿态。
...
5. 2:35–3:00：打开 `moonbit/agent/pkg.generated.mbti` 与 `moonbit/zhnum/zhnum_test.mbt` 的往返测试；说明固定教学骨架、三种动作、定性颜色与 AI 辅助开发边界。
...
- 为什么不是任意骨架引擎？`motion` 的姿态尺寸来自固定教学 rig，通用重定向尚未实现，文档已说明。
