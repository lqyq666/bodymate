...
**方向与使用者：** 开源生态库。`agent` 面向任何用 MoonBit 写 Agent / 工具调用 / 对话式控制的开发者；`zhnum` 面向中文 UI 指令、语音或聊天输入、配置与表单解析；`anatomy` 面向医学、体育、康复教育与可视化项目；`motion` 面向动作教学、动画/仿真状态机与需要可复现姿态帧的工具。
...
- `motion`：三个教学动作的参数契约、中文指令解析（经 zhnum）、独立 `Session`、四阶段讲解、确定性姿态意图。
...
**参考应用：** 离线全身三维实验室（415 肌肉、282 骨骼及相关结构、21 关节教学骨架），俯卧撑/深蹲/弯举的播放、参数比较、阶段讲解、697 个结构的中文点击与中英文检索，可选本地 AI 对话。页面上的每一条规则都来自上述 MoonBit 包，JavaScript 只编解码字符串。
...
**来源与限制：** 自有代码 MIT；人体几何保留 Human Atlas / BodyParts3D 的 CC BY 4.0 归属，不进入库包；Tripo 环境导出留在应用侧。AI 辅助开发如实披露。`motion` 姿态绑定固定教学骨架，`anatomy` 是展示译名；整套不提供医疗诊断、训练处方或实测发力结论。
