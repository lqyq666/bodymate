...
客户端会丢弃未知动作、未知参数、非有限数值和其他操作。这条规则的唯一实现是 MoonBit 包 `lqyq666/bodymate/agent`：本地代理在返回前、浏览器在执行前各调用一次同一份护栏（`src/ai/agent-guard.mjs` 只做字符串编解码）。真正启动动画、规范化参数和高亮参与肌群仍由现有 MoonBit 动作目录与 Three.js 查看器完成。高亮是定性参与提示，不是实测发力、肌电、诊断或训练处方。
