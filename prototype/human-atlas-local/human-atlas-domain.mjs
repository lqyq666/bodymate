...
export function visibilityForHumanAtlasSnapshot(snapshot, structureId) { return Object.freeze({ selectedVisible: !snapshot.isolated || snapshot.selected === structureId, contextVisible: !snapshot.isolated }); }
export function focusPlanFromBounds(bounds) {
  ...
  return Object.freeze({ target, distance: Math.max(diagonal * 1.9, 0.34) });
}
