...
test('model status derives real counts and UI actions use existing controllers', async () => {
  ...
  assert.match(full, /anatomy-phase-guide/);
  assert.match(full, /anatomy-parameter-comparison/);
  assert.match(full, /data-motion-phase/);
  assert.match(full, /viewer\.setPaused\(true\); viewer\.seek/);
