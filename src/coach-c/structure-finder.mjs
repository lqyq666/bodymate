...
export const coachActionNames = Object.freeze(['FIND_STRUCTURE', 'SELECT_STRUCTURE', 'ISOLATE_SELECTED', 'RESTORE_CONTEXT', 'SHOW_REGION', 'HIGHLIGHT_STRUCTURE_SET', 'CLEAR_STRUCTURE_SET', 'SHOW_MOVEMENT_MAPPING']);
...
const messageFor = (code, candidates, structureSet = null, movement = null) => ({ exact: `找到：${candidates[0]?.displayNameZh ?? '结构'}。`, structure_set: `已高亮${structureSet?.label ?? '结构集合'}（${structureSet?.members.length ?? 0} 块），焦点为${structureSet?.members[0]?.displayNameZh ?? '当前结构'}。`, movement: `${movement?.label ?? '该动作'}：当前模型覆盖 ${movement?.mappings.length ?? 0} 个参与结构。`, ambiguous: `这里可能对应 ${candidates.length} 块结构，选一块看看。`, uncovered: '目前真实模型只覆盖颈肩的 14 块结构，可以换个位置或名称试试。', health: '这里目前用于认识结构，不提供诊断或治疗建议。', isolate: '现在只显示当前结构。', restore: '周围结构已恢复。', empty: '输入一个颈肩结构名称、位置，或试试“只看这块”。' }[code] ?? '当前输入无法映射到已覆盖的结构。');
...
function parseMoonbitQueryV3(fields) {
  ...
  if (!['STRUCTURE_LOOKUP', 'STRUCTURE_SET_LOOKUP', 'MOVEMENT_LOOKUP', 'DOMAIN_COMMAND', 'UNSUPPORTED_HEALTH_QUERY', 'UNKNOWN'].includes(classification) || !['EXACT', 'AMBIGUOUS', 'NO_MATCH'].includes(resolution)) return null;
  const confidence = Number(confidenceRaw);
  ...
  let candidates = [], structureSet = null, movement = null;
  ...
  const action = actionName === 'UNKNOWN' ? null : Object.freeze({ schemaVersion: 1, action: actionName, ...(structureId ? { structureId } : {}), ...(region ? { region } : {}), ...(structureSet ? { structureSet } : {}), ...(movement ? { movement } : {}), confidence, source: 'moonbit-domain' });
  if (action && !isValidCoachActionContract(action)) return null;
  ...
  if (resolution === 'AMBIGUOUS' && (!action || action.action !== 'FIND_STRUCTURE' || candidates.length < 2)) return null;
  if (resolution === 'NO_MATCH' && (action || candidates.length || structureSet || movement)) return null;
  return Object.freeze({ classification, resolution, action, candidates, structureSet, movement, confidence: Number.isFinite(confidence) ? confidence : 0, message: messageFor(messageCode, candidates, structureSet, movement) });
}
